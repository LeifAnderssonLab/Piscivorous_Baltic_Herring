...
#Slåttersill etc frequency contrasts (backup)
...
#For Nat Comm revision
slatter_Gavle_pops <- grep("Gavle_large|Slattersill", names(merged_freq), value = T)
merged_freq$slatter_Gavle <- rowMeans(merged_freq[,slatter_Gavle_pops], na.rm = T)
merged_freq$Baltic_Spr_v_Slatter_Gavle <- abs(merged_freq$Baltic_Spr - merged_freq$slatter_Gavle)

transition_pops <- grep("HGS71_Rugen_Baltic_Spring|HGS72_Rugen_Baltic_Spring|TysklandS18_Germany_Baltic", names(merged_freq), value = T)
merged_freq$transition <- rowMeans(merged_freq[,transition_pops], na.rm = T)
merged_freq$transition_vs_SOK <- abs(merged_freq$transition - merged_freq$SOK)

Atl_transition_pops <- grep("Kattegat|Skagerrak|Hamburg", names(merged_freq), value = T)
merged_freq$transition_Atl <- rowMeans(merged_freq[,Atl_transition_pops], na.rm = T)
merged_freq$transition_Atl_vs_SOK <- abs(merged_freq$transition_Atl - merged_freq$SOK)
...
#For Nat Comm revision
png(filename = paste0("~/Projects/Herring/doc/Large_herring_WGS/WG_Baltic_Spr_vs_Slatter_Gavle.png"), width = 2000, height = 500 )
#pdf(file = paste0("~/Projects/Herring/doc/Large_herring_WGS/WG_Baltic_Spr_vs_Slatter_Gavle.pdf"), width = 15, height = 7.5 )
tmp_manhattan_roll_DAF <- stats::filter(merged_freq$Baltic_Spr_v_Slatter_Gavle[manhattan_site_filter], rep(1/50, 50))
plot(y = merged_freq$Baltic_Spr_v_Slatter_Gavle[manhattan_site_filter], 
     x = merged_freq$cumulative_pos[manhattan_site_filter],
     col = merged_freq$manhattan_col[manhattan_site_filter], pch = 20, cex = 0.3)
lines(y = tmp_manhattan_roll_DAF,
      x = merged_freq$cumulative_pos[manhattan_site_filter],
...
for(chr in paste0("chr", 12)){
  png(filename = paste0("~/Projects/Herring/doc/Large_herring_WGS/", chr, "_Transition_vs_SOK.png"), width = 1000, height = 500 )
  plot(y = merged_freq$transition_vs_SOK[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], x = merged_freq$POS[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], pch = 20, cex = 0.5)
  dev.off()
  png(filename = paste0("~/Projects/Herring/doc/Large_herring_WGS/", chr, "_Transition_Atl_vs_SOK.png"), width = 1000, height = 500 )
  plot(y = merged_freq$transition_Atl_vs_SOK[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], x = merged_freq$POS[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], pch = 20, cex = 0.5)
  dev.off()

  pdf(file = paste0("~/Projects/Herring/doc/Large_herring_WGS/", chr, "_Transition_vs_SOK.pdf"), width = 15, height = 7.5)
  plot(y = merged_freq$transition_vs_SOK[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], x = merged_freq$POS[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], pch = 20, cex = 0.5, xlab = "Position", ylab = "DAF", main = "SOK vs Transition zone (Baltic side; Han et al pops 15,16,18)")
  dev.off()
  pdf(file = paste0("~/Projects/Herring/doc/Large_herring_WGS/", chr, "_Transition_Atl_vs_SOK.pdf"), width = 15, height = 7.5 )
  plot(y = merged_freq$transition_Atl_vs_SOK[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], x = merged_freq$POS[merged_freq$CHR == chr & merged_freq$N_CHR >= 140], pch = 20, cex = 0.5, xlab = "Position", ylab = "DAF", main = "SOK vs Transition zone (Atlantic side; Han et al pops 27 - 30)")
  dev.off()
...
#WG manhattan for all 7 "Large" populations
large_pops <- c("Blekinge", "Finland", "Gavle_large", "Kalmar", "Ostergotland", "Slattersill", "Sthlm")
for(l_pop in large_pops){
  WG_file <- paste0("~/Projects/Herring/doc/Large_herring_WGS/WG_Baltic_Spr_vs_", l_pop, ".png")
  png(filename = WG_file, width = 2000, height = 500 )
  tmp_DAF_vec <- abs(merged_freq$Baltic_Spr - merged_freq[,l_pop])
  tmp_manhattan_roll_DAF <- stats::filter(tmp_DAF_vec[manhattan_site_filter], rep(1/50, 50))
  plot(y = tmp_DAF_vec[manhattan_site_filter], 
        x = merged_freq$cumulative_pos[manhattan_site_filter],
        col = merged_freq$manhattan_col[manhattan_site_filter], pch = 20, cex = 0.3)
  lines(y = tmp_manhattan_roll_DAF,
        x = merged_freq$cumulative_pos[manhattan_site_filter],
...
###Output of frequecies used for Manhattan plots
write("#Frequency data underlying Manhattan plots in Figures 3 and 4, as well as Supplementary Figure 1", file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/Frequency_and_DAF_contrast.tsv")
write.table(x = merged_freq[merged_freq$CHR == chr & merged_freq$N_CHR >= 140, ], file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/Frequency_and_DAF_contrast.tsv", sep = "\t", col.names = T, row.names = F, quote = F, append = T)
...
ext_pool_names <- names(merged_freq)[-hm_col_filter_ext]
...
chr20_scatter_filter_ext <- which(merged_freq$CHROM == "chr20" & merged_freq$POS > 7.5e6 & merged_freq$POS < 21.5e6 & merged_freq$N_CHR >= 150)
pdf("~/Projects/Herring/doc/Large_herring_WGS/Chr_20_15_Mb_DAF_ext.pdf", width = 15, height = 7)
plot(x = merged_freq$POS[chr20_scatter_filter_ext], y = merged_freq$British_v_SOK[chr20_scatter_filter_ext], pch = 20, ylim = c(0,1), cex = 0.6)
dev.off()

chr20_scatter_filter_whole <- which(merged_freq$CHROM == "chr20" & merged_freq$N_CHR >= 150)
pdf("~/Projects/Herring/doc/Large_herring_WGS/Chr_20_DAF.pdf", width = 15, height = 7)
plot(x = merged_freq$POS[chr20_scatter_filter_whole], y = merged_freq$British_v_SOK[chr20_scatter_filter_whole], pch = 20, ylim = c(0,1), cex = 0.6)
dev.off()
...
tmp_5_filter <- which(merged_freq$CHROM == "chr5" & merged_freq$N_CHR >= 140 & merged_freq$POS > 4e6 & merged_freq$POS < 4.5e6)
tmp_5_NonSyn_filter <- which(merged_freq$CHROM == "chr5" & merged_freq$N_CHR >= 140 & merged_freq$POS > 4e6 & merged_freq$POS < 4.5e6 & merged_freq$NonSyn)

plot(x = merged_freq$POS[tmp_5_filter], y = abs(merged_freq$Atlantic_Spr[tmp_5_filter]-merged_freq$Baltic_Spr[tmp_5_filter]))
points(x = merged_freq$POS[tmp_5_NonSyn_filter], y = abs(merged_freq$Atlantic_Spr[tmp_5_NonSyn_filter]-merged_freq$Baltic_Spr[tmp_5_NonSyn_filter]), pch = 20, col = "red")

abline(v = 4.191e6)

tmp_freq <- merged_freq[tmp_5_NonSyn_filter,-hm_col_filter_ext]
rownames(tmp_freq) <- paste(tmp_freq$CHROM, tmp_freq$POS, sep = "_")
pool_freq_hm_v3(tmp_freq, pool_order_vec = pool_order_vec_large_ext, margins = c(12, 15), pdf_file = "~/Projects/Herring/doc/Large_herring_WGS/Heatmaps/Chr5_4.2_Mb_red_opsin_NS_hm.pdf", col_lab_vec = rownames(tmp_freq))
...
##Revised version, including transition Zone
pool_order_vec_large_tra <- c(13,17,16,3,7,12,14,4,5,8,11,9,6,10)
pool_freq_hm_v3(tmp_freq, pool_order_vec = pool_order_vec_large_tra, margins = c(12, 15), pdf_file = "~/Projects/Herring/doc/Large_herring_WGS/Heatmaps/Chr_12_amacr_reg_hm_v2.pdf", col_lab_vec = rownames(tmp_freq))
write("#Frequency data underlying the heatmap in figure 4e", file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4e.tsv")
write(paste(c("SNP_id", names(tmp_freq)[pool_order_vec_large_tra]), collapse = "\t"), file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4e.tsv", append = T)
write.table(x = tmp_freq[, pool_order_vec_large_tra], file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4e.tsv", sep = "\t", col.names = F, row.names = T, quote = F, append = T)
...
##Revised version, including transition zone
pool_freq_hm_v3(tmp_freq, pool_order_vec = pool_order_vec_large_tra, margins = c(12, 15), pdf_file = "~/Projects/Herring/doc/Large_herring_WGS/Heatmaps/Chr_12_SOK_peak_1_hm_v2.pdf", col_lab_vec = rownames(tmp_freq))

write("#Frequency data underlying the heatmap in figure 4c", file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4c.tsv")
write(paste(c("SNP_id", names(tmp_freq)[pool_order_vec_large_tra]), collapse = "\t"), file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4c.tsv", append = T)
write.table(x = tmp_freq[, pool_order_vec_large_tra], file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4c.tsv", sep = "\t", col.names = F, row.names = T, quote = F, append = T)
###
...
##Revised version, including transition zone
pool_freq_hm_v3(tmp_freq, pool_order_vec = pool_order_vec_large_tra, margins = c(12, 15), pdf_file = "~/Projects/Herring/doc/Large_herring_WGS/Heatmaps/Chr_12_SOK_peak_2_hm_v2.pdf", col_lab_vec = rownames(tmp_freq))
write("#Frequency data underlying the heatmap in figure 4d", file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4d.tsv")
write(paste(c("SNP_id", names(tmp_freq)[pool_order_vec_large_tra]), collapse = "\t"), file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4d.tsv", append = T)
write.table(x = tmp_freq[, pool_order_vec_large_tra], file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig4d.tsv", sep = "\t", col.names = F, row.names = T, quote = F, append = T)
...
tmp_freq <- merged_freq[chr12_plot_filter,-hm_col_filter]
rownames(tmp_freq) <- paste(tmp_freq$CHROM, tmp_freq$POS, sep = "_")
pool_freq_hm_v3(tmp_freq, pool_order_vec = pool_order_vec_large_tra, margins = c(12, 15), pdf_file = "~/Projects/Herring/doc/Large_herring_WGS/Heatmaps/Chr_12_22Mb_hm_v2.pdf", col_lab_vec = rownames(tmp_freq))
#output heatmap data
write("#Frequency data underlying the heatmap in figure 3d", file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3d.tsv")
write(paste(c("SNP_id", names(tmp_freq)[pool_order_vec_large_tra]), collapse = "\t"), file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3d.tsv", append = T)
write.table(x = tmp_freq[, pool_order_vec_large_tra], file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3d.tsv", sep = "\t", col.names = F, row.names = T, quote = F, append = T)


##Frequency distance tree
chr12_tree_filter <- which(merged_freq$CHROM == "chr12" & merged_freq$POS > 17e6 & merged_freq$POS < 27e6 & merged_freq$N_CHR >= 140)
tmp_tree_freq <- merged_freq[chr12_tree_filter,-hm_col_filter]
tmp_tree_freq <- tmp_tree_freq[, pool_order_vec_large_tra]
...
chr12_raw_dist <- outer(X = 1:dim(tmp_tree_freq)[2], Y = 1:dim(tmp_tree_freq)[2], FUN = "f3", data_mat = tmp_tree_freq)
rownames(chr12_raw_dist) <- names(tmp_tree_freq)[1:dim(tmp_tree_freq)[2]]
colnames(chr12_raw_dist) <- names(tmp_tree_freq)[1:dim(tmp_tree_freq)[2]]

save(chr12_raw_dist, file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/chr12_raw_dist.RData")

require(ape)
chr12_dist <- as.dist(chr12_raw_dist)
chr12_tree <- bionj(chr12_dist)
chr12_tree$tip.label <- paste0("___", colnames(chr12_raw_dist), "___")

#png(file = "~/Projects/Herring/doc/Large_herring_WGS/chr12_inv_pool_tree.png", height = 1000, width = 1000)
pdf(file = "~/Projects/Herring/doc/Large_herring_WGS/chr12_inv_pool_tree.pdf", height = 10, width = 10)
plot.phylo(chr12_tree, type="unrooted", lab4ut = "axial", cex = 2, edge.width = 2.5)
...
tmp_freq <- merged_freq[chr20_plot_filter,-hm_col_filter]
rownames(tmp_freq) <- paste(tmp_freq$CHROM, tmp_freq$POS, sep = "_")
pool_freq_hm_v3(tmp_freq, pool_order_vec = pool_order_vec_large_tra, margins = c(12, 15), pdf_file = "~/Projects/Herring/doc/Large_herring_WGS/Heatmaps/Chr_20_15Mb_hm_v2.pdf", col_lab_vec = rownames(tmp_freq))
write("#Frequency data underlying the heatmap in figure 3c", file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3c.tsv")
write(paste(c("SNP_id", names(tmp_freq)[pool_order_vec_large_tra]), collapse = "\t"), file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3c.tsv", append = T)
write.table(x = tmp_freq[, pool_order_vec_large_tra], file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3c.tsv", sep = "\t", col.names = F, row.names = T, quote = F, append = T)


##Frequency distance tree
chr20_tree_filter <- which(merged_freq$CHROM == "chr20" & merged_freq$POS > 12.5e6 & merged_freq$POS < 17e6 & merged_freq$N_CHR >= 140)
tmp_tree_freq <- merged_freq[chr20_tree_filter,-hm_col_filter]
tmp_tree_freq <- tmp_tree_freq[, pool_order_vec_large_tra]

chr20_raw_dist <- outer(X = 1:dim(tmp_tree_freq)[2], Y = 1:dim(tmp_tree_freq)[2], FUN = "f3", data_mat = tmp_tree_freq)
rownames(chr20_raw_dist) <- names(tmp_tree_freq)[1:dim(tmp_tree_freq)[2]]
colnames(chr20_raw_dist) <- names(tmp_tree_freq)[1:dim(tmp_tree_freq)[2]]

require(ape)
chr20_dist <- as.dist(chr20_raw_dist)
chr20_tree <- bionj(chr20_dist)
chr20_tree$tip.label <- paste0("___", colnames(chr20_raw_dist), "___")

#png(file = "~/Projects/Herring/doc/Large_herring_WGS/chr20_pool_tree.png", height = 1000, width = 1000)
pdf(file = "~/Projects/Herring/doc/Large_herring_WGS/chr20_pool_tree.pdf", height = 10, width = 10)
plot.phylo(chr20_tree, type="unrooted", lab4ut = "axial", cex = 2, edge.width = 2.5)
add.scale.bar(length = NULL, ask = FALSE,
              lwd = 1, lcol = "black")
dev.off()

##
write.nexus(chr20_tree,file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3c.nex")
write.nexus(chr12_tree,file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3e.nex")


##Backgroud tree
bg_tree_filter <- which(merged_freq$N_CHR >= 140)
bg_tree_filter <- bg_tree_filter[sample(x = length(bg_tree_filter), size = 5e5)]
tmp_tree_freq <- merged_freq[bg_tree_filter,-hm_col_filter]
tmp_tree_freq <- tmp_tree_freq[, pool_order_vec_large_tra]

bg_raw_dist <- outer(X = 1:dim(tmp_tree_freq)[2], Y = 1:dim(tmp_tree_freq)[2], FUN = "f3", data_mat = tmp_tree_freq)
rownames(bg_raw_dist) <- names(tmp_tree_freq)[1:dim(tmp_tree_freq)[2]]
colnames(bg_raw_dist) <- names(tmp_tree_freq)[1:dim(tmp_tree_freq)[2]]

require(ape)
bg_dist <- as.dist(bg_raw_dist)
bg_tree <- bionj(bg_dist)
bg_tree$tip.label <- paste0("___", colnames(bg_raw_dist), "___")

png(file = "~/Projects/Herring/doc/Large_herring_WGS/background_pool_tree.png", height = 1000, width = 1000)
plot.phylo(bg_tree, type="unrooted", lab4ut = "axial", cex = 2, edge.width = 2.5)
add.scale.bar(length = NULL, ask = FALSE,
              lwd = 1, lcol = "black")
dev.off()
...
#merged_freq_PCA_selected_raw <- merged_freq[merged_freq$join_check_SNP_ID %in% selected_han_SNPs$SNP_ID,ext_pool_names]
merged_freq_PCA_selected_raw <- merged_freq[merged_freq$join_check_SNP_ID %in% selected_han_SNPs$SNP_ID,ext_pool_names[-c(1,2)]]
merged_freq_PCA_selected <- merged_freq_PCA_selected_raw[,-grep("HWS|Pacific", names(merged_freq_PCA_selected_raw))]
...
var_explained <- mf_sel.pca$sdev^2/sum(mf_sel.pca$sdev^2)
...
round(var_explained*100, digits = 1)
...
mf.pca_df$plot_col[grep("Gavle_large|Kalmar$|Ostergotland|Sthlm|Blekinge|Finland", rownames(mf.pca_df))] <- "turquoise3"
mf.pca_df$plot_col[grep("Slattersill", rownames(mf.pca_df))] <- "limegreen"

mf.pca_df$label <- ""
mf.pca_df$label[grep("Slattersill", rownames(mf.pca_df))] <- "Sl"
mf.pca_df$label[grep("Gavle_large", rownames(mf.pca_df))] <- "G"
mf.pca_df$label[grep("Kalmar$", rownames(mf.pca_df))] <- "K"
mf.pca_df$label[grep("Ostergotland", rownames(mf.pca_df))] <- "Ö"
mf.pca_df$label[grep("Sthlm", rownames(mf.pca_df))] <- "St"
mf.pca_df$label[grep("Blekinge", rownames(mf.pca_df))] <- "Bl"
mf.pca_df$label[grep("Finland", rownames(mf.pca_df))] <- "F"

mf.pca_df$plot_pos <- 3
mf.pca_df$plot_pos[rownames(mf.pca_df) %in% c("Kalmar")] <- 1
mf.pca_df$plot_pos[rownames(mf.pca_df) %in% c("Sthlm")] <- 2
mf.pca_df$plot_pos[rownames(mf.pca_df) %in% c("Ostergotland")] <- 3
mf.pca_df$plot_pos[rownames(mf.pca_df) %in% c("Blekinge", "Gavle_large", "Slattersill","Finland")] <- 4
...
plot(x = mf.pca_df$PC1, y = mf.pca_df$PC2, pch = 20, col = mf.pca_df$plot_col, cex = 3, xlab = "PC1", ylab = "PC2")
text(x = mf.pca_df$PC1, y = mf.pca_df$PC2, labels = mf.pca_df$label, cex = 1.4, pos = mf.pca_df$plot_pos)
legend(x = "topleft", 
       legend = c("Slåttersill", "Large (other)", "Spring", "Autumn"),
       col = c("limegreen", "turquoise3", "indianred3", "black"), pch = 20, cex = 2, bty = "n")
dev.off()

##
write.table(mf.pca_df, file = "~/Projects/Herring/doc/Large_herring_WGS/NatCom_uploads/fig3a.tsv", row.names = F, col.names = T, sep = "\t", quote = F)

### Counts-based Chi-square test

#example_file_name <- "~/Projects/Herring/data/slattersill_etc/counts/Sthlm.frq.count"
#example_counts <- read.table(file = example_file_name, header = F, skip = 1, sep = "\t", stringsAsFactors = F)
#names(example_counts) <- c("CHROM", "POS", "N_ALLELES", "N_CHR", "REF_COUNT", "ALT_COUNT")
#ex_sample_ID <- sub(".+[/]([0-9A-Za-z_]+)[.]frq[.]count", "\\1", example_file_name)
#example_counts[,ex_sample_ID] <- paste(example_counts$REF_COUNT, example_counts$ALT_COUNT, sep = ",")

count_files <- grep("frq[.]count", dir("~/Projects/Herring/data/slattersill_etc/counts", full.names = T), value = T)
for(cf in count_files){
  
  current_counts <- read.table(file = cf, header = F, skip = 1, sep = "\t", stringsAsFactors = F)
  current_sample_ID <- sub(".+[/]([0-9A-Za-z_]+)[.]frq[.]count", "\\1", cf)
  names(current_counts) <- c("CHROM", "POS", "N_ALLELES", "N_CHR", "REF_COUNT", "ALT_COUNT")
  current_counts[,current_sample_ID] <- paste(current_counts$REF_COUNT, current_counts$ALT_COUNT, sep = ",")
  current_counts[,paste0(current_sample_ID, "_n_chr")] <- current_counts$N_CHR
  
  if(cf == count_files[1]){
    merged_count_df <- current_counts
  } else{
    merged_count_df <- full_join(merged_count_df, current_counts[,c("CHROM", "POS",current_sample_ID, paste0(current_sample_ID, "_n_chr"))], by = c("CHROM", "POS"))
  }
}
save(merged_count_df, file = "~/Projects/Herring/data/slattersill_etc/counts/merged_counts.RData")

#Importing older population counts
Han_et_al_counts <- read.table(file = "~/Projects/Herring/data/slattersill_etc/counts/60.Neff.AD.gz" , header = T, sep = "\t", stringsAsFactors = F)
save(Han_et_al_counts, file = "~/Projects/Herring/data/slattersill_etc/counts/Han_et_al_counts.RData")


#test_count_df <- head(merged_count_df, 100)
#test_pop1 <- c("Sthlm", "Kalmar", "Ostergotland")
#test_pop2 <- c("Gavle_large", "Slattersill")
test_pop_chisq <- pop_allele_chisq(count_df = test_count_df, pop1 = test_pop1, pop2 = test_pop2)

load("~/Projects/Herring/data/slattersill_etc/counts/Large_v_Spr_pop_chisq.RData")

Large_v_Spr_pop_chisq$cumulative_pos <- NA
Large_v_Spr_pop_chisq$cumulative_pos <- Large_v_Spr_pop_chisq$POS + Ch_v2.0.2_sizes$offset[match(Large_v_Spr_pop_chisq$CHROM, Ch_v2.0.2_sizes$name)] 
Large_v_Spr_pop_chisq$manhattan_col <- Ch_v2.0.2_sizes$col[match(Large_v_Spr_pop_chisq$CHROM, Ch_v2.0.2_sizes$name)] 
Large_v_Spr_pop_chisq <- Large_v_Spr_pop_chisq[grepl("chr",Large_v_Spr_pop_chisq$CHROM),]

png(filename = "~/Projects/Herring/doc/Large_herring_WGS/Large_v_Spr_chisq_WG.png", width = 2000, height = 500 )
  plot(y = -log10(Large_v_Spr_pop_chisq$chisq_p), 
     x = Large_v_Spr_pop_chisq$cumulative_pos,
     col = Large_v_Spr_pop_chisq$manhattan_col, pch = 20, cex = 0.3)
dev.off()

chisq_files <- grep("chisq[.]RData", dir("~/Projects/Herring/data/slattersill_etc/counts", full.names = T), value = T)

#for(chi_f in chisq_files){ #The full batch
for(chi_f in chisq_files[8]){ #A particular contrast
  current_chisq <- load(chi_f)
  tmp_chisq_df <- get(current_chisq)
  rm(list = current_chisq)
  tmp_chisq_df$cumulative_pos <- NA
  tmp_chisq_df$cumulative_pos <- tmp_chisq_df$POS + Ch_v2.0.2_sizes$offset[match(tmp_chisq_df$CHROM, Ch_v2.0.2_sizes$name)] 
  tmp_chisq_df$manhattan_col <- Ch_v2.0.2_sizes$col[match(tmp_chisq_df$CHROM, Ch_v2.0.2_sizes$name)] 
  tmp_chisq_df <- tmp_chisq_df[grepl("chr",tmp_chisq_df$CHROM),]

  png(filename = paste0("~/Projects/Herring/doc/Large_herring_WGS/",current_chisq,"_WG.png"), width = 2000, height = 500 )
    plot(y = -log10(tmp_chisq_df$chisq_p), 
      x = tmp_chisq_df$cumulative_pos,
      col = tmp_chisq_df$manhattan_col, pch = 20, cex = 0.3)
  dev.off()
}

##QQ plot
obs_p <- -log10(tmp_chisq_df$chisq_p)
obs_p <- obs_p[order(obs_p)]
exp_p <- -log10(seq(from = 1/length(tmp_chisq_df$chisq_p), to = 1-(1/length(tmp_chisq_df$chisq_p)), length.out = length(tmp_chisq_df$chisq_p)))
exp_p <- exp_p[order(exp_p)]
lambda_est <- lm(obs_p[1:floor(length(obs_p)*0.9)]~exp_p[1:floor(length(obs_p) * 0.9)] - 1)
  
png(filename = paste0("~/Projects/Herring/doc/Large_herring_WGS/",current_chisq,"_QQ.png"), width = 2000, height = 500 )
plot(y = obs_p, x = exp_p, pch = 20, cex = 0.2, xlim = c(0,10), ylim = c(0,10))
abline(reg = lambda_est, col = "red")
dev.off()
